idesignresAPP.controller("RolesSummaryController", function($scope, $state, $stateParams, $http, $window, $translate, modalFactory, routeFactory, sessionFactory) {
	// Init factories
	...
	// Function: doLoadRole
	$scope.doLoadRole = function() {
		// Call service
		$scope.ready = false;
		$scope.notReadyText = $translate.instant('roles.wait.load');
		$http({
			method  : 'GET',
		    url     : $scope.backendUrl + $scope.apiRolesUuid.replace('{uuid}', $stateParams.uuid),
		    headers : {
		...
		.success(function(response) {
			...
			$scope.roleObj = angular.fromJson(response.data);
			if ($scope.roleObj == null) {
				$scope.modalFactory.doOpenModalInfo($translate.instant('global.error404'), $translate.instant('roles.error.noRole'));
				$scope.routeFactory.goRoles($scope.currentPage);
			}
		})
	...
	$scope.$on('$viewContentLoaded', function() {
		...
		$http.get('/config/config.json').then(function(response) {
			...
			// Define API objects
			$scope.apiRolesUuid = response.data.api.apiRolesUuid;
