...
def pv1Step01(layer, nutsId, config):
    ...
    logging.info('  QGIS Server/> PV Power Plants -> Preprocess -> Step 01 -> Processing "' + layerName + '"...')
    ...
    logging.info('  QGIS Server/> PV Power Plants -> Preprocess -> Step 01 -> NUTS name -> ' + nutsName)
...
def pv1Steps02030405(layer, mask, step, config):
    ...
    layerName = layer['name'] + '.' + layer['format']
    logging.info('  QGIS Server/> PV Power Plants -> Preprocess -> Step ' + step + ' -> Processing "' + layerName + '"...')
...
def pv1Step07(srcPath, config):
    ...
    logging.info('  QGIS Server/> PV Power Plants -> Preprocess -> Step 07 -> User codes -> ' + str(userCodes))
...
def pv1Step15(radiationPotentialAreas, nutsOutputPath, process, username, nutsid, config):
    ...
    logging.info('  QGIS Server/> PV Power Plants -> Preprocess -> Step 15 -> Temp. output file -> ' + csvName)
